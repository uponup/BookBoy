...
struct NoteDao {
    ...
    static func addNote(note: Note) {
        let sql = "INSERT INTO NoteTable(title, content, book_id, img) VALUES (?, ?, ?, ?);"
        let db = SQLiteManager.sharedManager().db
        if db.open() {
            if db.executeUpdate(sql, withArgumentsIn: [note.title, note.content, note.bookId, note.img as Any, note.isComplete]) {
                print("插入成功")
            }else {
                print("插入失败")
            }
